...
<div x-transition class="pt-3" x-cloak x-show="{{ $show }}" x-anchor="{{ $ref }}" x-on:click.outside="{{ $show }} = false">
    <div class="bg-white rounded-lg shadow-lg border max-w-sm overflow-clip">
        {{ $slot }}
    </div>
</div>

<div x-transition class="pt-1 px-4" x-cloak x-show="{{ $show }}" x-anchor="{{ $ref }}">
    <div class="relative">
